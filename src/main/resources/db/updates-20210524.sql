...
(
    ...
    is_delete   tinyint(2)  not null default 0 comment '已删除'
) engine = InnoDB
...
(
    ...
    contain_path_var tinyint(2)   not null default 0 comment '包含path variable.',
...
(
    ...
    instrument_memo   varchar(2040) not null default '' comment '仪器备注',
    is_idle           tinyint(2)    not null default 1 comment '是否空闲',
    is_available      tinyint(2)    not null default 1 comment '是否可用',
    location          varchar(100)  not null default '' comment '实验室位置',
    open_time         varchar(5)    not null default '00:00' comment '开放时间 24小时制',
    close_time        varchar(5)    not null default '23:59' comment '关闭时间 24小时制',
    memo              text comment '备注',
    create_time       datetime      not null default current_timestamp() comment '创建时间',
    create_user_id    int           not null default 0 comment '创建人id',
    create_username   varchar(100)  not null default '' comment '创建人name',
    update_time       datetime      not null default current_timestamp() on update current_timestamp comment '更新时间',
    update_user_id    int           not null default 0 comment '更新人id',
    update_username   varchar(100)  not null default '' comment '更新人name',
    is_delete         tinyint(2)    not null default 0 comment '已删除'
) engine = InnoDB
...
(
    ...
    no_booking      tinyint(2)   not null default 1 comment '未预约实验室',
    memo            text comment '备注',
    create_time     datetime     not null default current_timestamp() comment '创建时间',
    create_user_id  int          not null default 0 comment '创建人id',
    create_username varchar(100) not null default '' comment '创建人name',
